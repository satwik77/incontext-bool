...
from src.components.blocks.hyena_encoder import Hyena



class HyenaModel(nn.Module):
	def __init__(self, n_dims, d_model, n_layer, order = 3, dropout=0.0):
		super(HyenaModel, self).__init__()
		self.model_type = 'Hyena'
	
		self.pos_encoder = LearnablePositionalEncoding(d_model, dropout)

		self.name = f"hyena_model={d_model}_layer={n_layer}_head={order}"
		...
		self._backbone= Hyena(d_model=d_model, n_layers=n_layer, order= order, d_ffn=d_ffn)
		self._read_out = nn.Linear(d_model, 1)

		print('Hyena Normal Training: All parameters are tunable')
